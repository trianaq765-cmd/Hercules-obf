...
<head>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        textarea{width:100%;height:200px;padding:15px;border:2px solid rgba(255,107,107,0.3);border-radius:10px;background:rgba(0,0,0,0.3);color:#fff;font-family:monospace;font-size:14px}
    ...
    async function obfuscate() {
        ...
        try {
            ...
            if (data.success) {
                ...
                status.textContent = '✅ Done! Time: ' + data.time_taken;
...
def api_obfuscate():
    try:
        ...
        if output_file:
            ...
            return jsonify({'success': True, 'output': output, 'time_taken': f'{elapsed:.2f}s', 'original_size': len(code), 'obfuscated_size': len(output)})
        else:
